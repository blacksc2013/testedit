! function(e) {
    ...
    e("[data-bg-src]").length > 0 && e("[data-bg-src]").each((function() {
        ...
        function d(e) {
            var t = e.realIndex + 1,
                a = e.slides.length;
            i.html('<span class="current-slide">' + (t < 10 ? "0" + t : t) + '</span> <span class="divider">/</span> <span class="total-slides">' + (a < 10 ? "0" + a : a) + "</span>")
        }
        ...
        function h(t) {
            ...
            c && s.css("background-image", "url(" + c + ")"), l && n.css("background-image", "url(" + l + ")")
        }
        ...
        if (a.length) {
            var n = a[0].swiper;
            n && (e(this).data("slider-prev") ? n.slidePrev() : (navigator, n.slideNext()))
        }
    }));
    ...
    ! function() {
        ...
        i.on("slideChange", (function() {
            ...
            s && (t.style.backgroundImage = s.style.backgroundImage), o && (a.style.backgroundImage = o.style.backgroundImage)
        }))
                ...
                function a() {
                    requestAnimationFrame(a), document.querySelectorAll(".single").forEach(((a, n) => {
                        const s = a.getBoundingClientRect(),
                            o = .5 * window.innerWidth - (s.x + .5 * s.width);
                        let i = Math.abs(o) * e - s.width * e;
                        i < 0 && (i = 0);
                        const r = o < 0 ? "left top" : "right top";
                        a.style.transform = `translate(0, ${i}px) rotate(${-o*t}deg)`, a.style.transformOrigin = r
                    }))
                }()
        }))
        ...
        return this.each((function() {
            ...
            if (n.on("click", (function(t) {
                    t.preventDefault();
                    var n = e(this);
                    if (n.addClass("active").siblings().removeClass("active"), l(n), a.sliderTab) {
                        var s = n.index();
                        i.slideTo(s)
                    }
                })), a.sliderTab) {
    ...
    function m() {
        var t, a = e(d).serialize();
        t = function() {
            var t, a = !0;

            function n(n) {
                n = n.split(",");
                for (var s = 0; s < n.length; s++) t = d + " " + n[s], e(t).val() ? (e(t).removeClass(u), a = !0) : (e(t).addClass(u), a = !1)
            }
            n(h), e(p).val() && e(p).val().match(/^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/) ? (e(p).removeClass(u), a = !0) : (e(p).addClass(u), a = !1);
            return a
        }(), t && jQuery.ajax({
            url: e(d).attr("action"),
            data: a,
    ...
    e(d).on("submit", (function(e) {
        ...
        e(this).each((function() {
            var n, s, o, i, r, c = e(this);
            n = Math.floor(c.height() / 2), s = c.attr(t), o = c.attr(a), i = v(e(o).css("padding-top")), r = v(e(o).css("padding-bottom")), "top-half" === s ? (e(o).css("padding-bottom", r + n + "px"), c.css("margin-top", "-" + n + "px")) : "bottom-half" === s && (e(o).css("padding-top", i + n + "px"), c.css("margin-bottom", "-" + n + "px"))
        }))
    };
    var y = "[data-sec-pos]";
    e(y).length && e(y).imagesLoaded((function() {
        ...
        e(this).each((function() {
            ...
            var s = setInterval((function() {
                ...
                i < 10 && (i = "0" + i), r < 10 && (r = "0" + r), c < 10 && (c = "0" + c), l < 10 && (l = "0" + l), o < 0 ? (clearInterval(s), t.addClass("expired"), t.find(".message").css("display", "block")) : (n(".day").html(i), n(".hour").html(r), n(".minute").html(c), n(".seconds").html(l))
            }), 1e3)
        }))
    ...
    window.addEventListener("pointermove", (e => {
        ...
        format: "H:i",
        step: 30
    }), e(".date-time-pick").datetimepicker({}), document.addEventListener("DOMContentLoaded", (function() {
        ...
        e.forEach(((e, n) => {
            e.addEventListener("mouseenter", (function() {
                ...
                document.querySelector(`.product-hotspot-dot${n+1}`).classList.add("show"), a = n + 1
            }))
        })), t.forEach((e => {
            e.addEventListener("mouseenter", (function() {
   ...
   // enable it before production
    ...
    // Messenger Widget Functionality
    ...
    //     if (messengerToggle && messengerPanel) {
    ...
    //     document.addEventListener('click', function(e) {
    //         if (messengerPanel && messengerPanel.classList.contains('active')) {
    //             if (!messengerPanel.contains(e.target) && !messengerToggle.contains(e.target)) {
    //                 messengerPanel.classList.remove('active');
    //             }
    //         }
    //     });
    ...
    //     // Auto-hide notification badge when panel is opened
    //     if (messengerToggle && messengerPanel) {
    //         messengerToggle.addEventListener('click', function() {
    //             const notificationBadge = this.querySelector('.notification-badge');
    //             if (notificationBadge && messengerPanel.classList.contains('active')) {
    //                 notificationBadge.style.display = 'none';
    //             }
    //         });
    //     }
        
    //     // Fix client slider auto-scroll direction
    //     setTimeout(function() {
    //         const clientSlider = document.getElementById('clientSlider7');
    //         if (clientSlider && clientSlider.swiper) {
    //             // Force the swiper to start from the correct position
    //             clientSlider.swiper.slideTo(0, 0, false);
                
    //             // Start autoplay if it's not already running
    //             if (!clientSlider.swiper.autoplay.running) {
    //                 clientSlider.swiper.autoplay.start();
    //             }
                
    //             // Ensure proper direction and speed
    //             clientSlider.swiper.params.speed = 2000;
    //             clientSlider.swiper.params.autoplay.delay = 3000;
    //             clientSlider.swiper.params.autoplay.disableOnInteraction = false;
    //             clientSlider.swiper.params.autoplay.pauseOnMouseEnter = false;
                
    //             // Force immediate slide to next position to start the right direction
    //             setTimeout(function() {
    //                 if (clientSlider.swiper && clientSlider.swiper.autoplay.running) {
    //                     clientSlider.swiper.slideNext();
    //                 }
    //             }, 100);
    //         }
    //     }, 1000);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const swiperElement = document.querySelector('.team-slider1'); // Adjust selector to match your Swiper container
  const swiper = swiperElement.swiper; // Get the Swiper instance

  // Pause autoplay on touch start (mobile)
  swiperElement.addEventListener('touchstart', function () {
    swiper.autoplay.stop();
    console.log('Slider paused on touch');
  });

  // Resume autoplay on touch end (mobile, simulates "lost focus")
  swiperElement.addEventListener('touchend', function () {
    swiper.autoplay.start();
    console.log('Slider resumed on touch end');
  });
  ...
  // Resume autoplay on mouse leave (desktop, simulates "lost focus")
